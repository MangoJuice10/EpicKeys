<div name="menu" class="relative border-b-1 border-[var(--color-border)]">
    <div class='flex justify-between h-[75px] px-[75px] bg-[var(--color-1)]'>
        ...
        <div name='links' class='flex justify-between items-center gap-[40px]'>
            ...
            <div class="group self-stretch flex items-center">
                ...
                <div x-data="{ selectedGenre: '{{ __('static.genres.all') }}' }"
                    class="hidden group-hover:grid hover:grid grid-cols-4 grid-rows-1 absolute top-full w-[850px] h-[596px] border-1 border-[var(--color-border)] bg-[var(--color-1)] z-1">
                    <div class="col-span-1 grid border-r-1 border-[var(--color-border)]">
                        <x-genre name="{{ __('static.genres.all') }}"
                            x-on:mouseenter="selectedGenre = '{{ __('static.genres.all') }}'"></x-genre>
                        @foreach ($genres as $genre)
                            <x-genre wire:key="genre-{{ $genre->id }}" x-on:mouseenter="selectedGenre = '{{ $genre->name }}'"
                    ...
                    <div class="col-span-3 flex flex-col items-center gap-[40px] px-[30px]">
                        ...
                        <x-cards.image-rectangle-card name="Red Dead Redemption 2" minPrice="900"
                            availableRegions="Аргентина, США" imagePath="{{ Storage::url('games/icons/rdr2.jpg') }}" />
                        <span
                            class="flex justify-center items-center min-w-[536px] min-h-[45px] border-1 border-[var(--color-border)] bg-[var(--color-2)] tracking-[1px] cursor-pointer">Просмотреть
        ...
        <div class="self-end flex justify-between gap-[40px] h-full">
            <div class="flex justify-center items-center">
                <div name='search'
                    class='flex justify-start items-center gap-[6px] h-[26px] w-[246px] overflow-x-auto px-[20px] border-1 rounded-3xl border-[var(--color-border)] bg-white cursor-pointer'>
            ...
            <div class="flex justify-between items-center gap-[20px]">
                <div class="self-stretch flex items-center group">
                    <span>{{ strtoupper(app()->getLocale()) }}</span>
                    <div class="hidden group-hover:block hover:block absolute top-full w-full">
                        <div
                            class="grid grid-cols-1 justify-stretch items-stretch divide-y-[1px] divide-[var(--color-border)] relative -left-[10px] border-1 border-[var(--color-border)] bg-[var(--color-1)]">
                            <x-language imagePath='images/flags/ru.svg'>ru</x-language>
                            <x-language imagePath='images/flags/en.svg'>en</x-language>
                        </div>
                    </div>
                </div>
                <div name="user" class="cursor-pointer">
